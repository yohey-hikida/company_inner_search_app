...
def initialize_retriever():
    ...
    # チャンク分割を実施（社員名簿は分割せず、その他はチャンク分割）
    splitted_docs = []
    for doc in docs_all:
...
def file_load(path, docs_all):
    ...
    if file_extension == '.csv' and file_name == '社員名簿.csv':
        ...
        if len(rows) >= 2:
            ...
            content = "\n".join(content_lines)
            docs_all.append(LcDocument(page_content=content, metadata={"source": path}))
        # unified csv doc created, skip default loader
        return
